...
module.exports = (function() {
  ...
  var Adapter = {
    ...
    addTo: function(connection, collection, options, cb) {
      var target = assocTarget(connection, collection, options);
      return Adapter.sailsIoSocket(connection, collection, 'post', target.url, target.data, cb);
    },
    ...
    removeFrom: function(connection, collection, options, cb) {
      var target = assocTarget(connection, collection, options);
      return Adapter.sailsIoSocket(connection, collection, 'delete', target.url, target.data, cb);
    },
     ...
     * @param {String} connection The connection identifier of the collection being queried.
     * @param {String} collection The identity of the collection being queried.
     * @param {String} method A REST method.
     * @param {String} url
    ...
    sailsIoSocket: function(connection, collection, method, url, data, cb) {
      ...
      if (!cb) {
        return new Promise(function(resolve, reject) {
          ...
          if (process.env.NODE_ENV !== "production") {
            var executeNow = function() {
              conn.socketRequest(method, url, data, function(err, result) {
   ...
   */
  function assocTarget(connection, collection, options) {
    if (process.env.NODE_ENV !== "production") {
    ...
    var target = {
      url: grabConnection(connection).basePath + collection + '/' + options.id + '/'+ options.association
    };
