...
Socket.prototype.select = function(collectionName, options, cb) {
  ...
  this.io.socket.get(this.config.pathname + '/' + collectionName, options, function(data) {
    cb(null, data);
  });
...
Socket.prototype.insert = function(collectionName, values, cb) {
  ...
  this.io.socket.post(this.config.pathname + '/' + collectionName, values, function(data) {
    cb(null, data);
  });
...
Socket.prototype.update = function(collectionName, options, values, cb) {
  ...
  if (id) {
    ...
    this.io.socket.put(path + '/' + id, values, function(data) {
      cb(null, data);
    });
  } else {
    // Handling multiple updates requires the additional custom Blueprint findAndUpdate route
    options.data = values;
    this.io.socket.put(path, options, function(data) {
      cb(null, data);
    });
  }
...
Socket.prototype.destroy = function(collectionName, options, cb) {
  ...
  this.io.socket.delete(path, options, function(data) {
    cb(null, data);
  });
